...
// const oauthDropdown = document.getElementById('oauth-template').content.querySelector('.oauth-dropdown');
// const oauthTable = document.getElementById('oauth-template').content.querySelector('.oauth-table');
// console.log(oauthDropdown, oauthTable)
// oauthDropdown.addEventListener("click", authDropdown, false);

// function authDropdown() {
//     console.log(2)
//     oauthTable.style.display = 'none';
// }
    
const clientId = '69e77828294d4f13b0500d5a164f9ba7';
// const clientSecret = ;

const redirectUrl = 'http://localhost:5500/spotifyUserData.html';     
...
if (currentToken.access_token) {
    ...
    renderTemplate("followed-artists", "followed-artists-template", followedArtists);
    renderTemplate("saved-tracks", "saved-tracks-template", savedTracks);
...
async function getFollowedArtists() {
    async function repeatGet(url, followedTotal = []) {
        ...
        followedObject = await response.json();
...
async function getSavedTracks() {
    async function repeatGet(url, followedTotal = []) {
        // if (typeof(followedTotal)==='undefined') {followedTotal=[]}
        const response = await fetch(url, {
            method: 'GET',
            headers: { 'Authorization': 'Bearer ' + currentToken.access_token },
        });
        ...
        followedObject = await response.json();
        for (artist of followedObject.items) { followedTotal.push(artist) }

        if (followedObject.next != undefined) { }//await repeatGet(followedObject.next, followedTotal) }

        return { followedTotal }
    }
    followedTotal = await repeatGet("https://api.spotify.com/v1/me/tracks?limit=50")
    return {"tracks":followedTotal}
}
...
// document.querySelector(".fff").addEventListener("click", refreshTokenClick, false)
